...
export default (props) => {
  ...
  const {
    ...
    raw_capacity: reportedRawCapacity,
    domains_quota: domainsQuota,
    backend_quota: backendQuota,
    usage,
    burst_usage: burstUsage,
    unit: unitName,
  } = props.resource
  ...
  if (rawCapacity > 0 && rawCapacity < capacity) {
    ...
    barProps.beforeOvercommitTooltip = `Raw capacity = ${unit.format(
      rawCapacity
    )}`
  ...
  if (domainsQuota > capacity) {
    infoMessage = (
      <span className="resource-error text-danger">
        <i className="fa fa-lg fa-warning" /> Quota assignments exceed measured
        capacity
      </span>
    )
  }
    ...
    <div className="row">
      ...
      <div className={props.wide ? "col-md-9" : "col-md-5"}>
        <ResourceBar
          ...
          {...barProps}
          unitName={unitName}
          isDanger={barProps.fill > capacity}
          showsCapacity={true}
        />
      </div>
      {!props.wide && (
        <div className="col-md-5">
          <Link
            to={`/${props.area}/details/${props.categoryName}/${resourceName}`}
            className="btn btn-primary btn-sm btn-quota-details"
          >
            Show domains
          </Link>
          {infoMessage}
